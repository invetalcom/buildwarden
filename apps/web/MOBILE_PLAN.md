...
- **The mobile UI ships from two build outputs, and changing web source updates neither on its own.**
  `apps/desktop/out/web` is only regenerated by `pnpm --filter @buildwarden/web build:embedded` (or a
  full `pnpm --filter @buildwarden/desktop build`); `electron-vite dev` does *not* rebuild it. A phone
  paired to the desktop host therefore keeps serving the last embedded build until that command is
  re-run, which reads exactly like the change never landed. When a mobile change appears to have no
  effect, check which of the three surfaces is being served — Vite dev server, `apps/web/dist`
  (Vercel), or `apps/desktop/out/web` (desktop host) — before debugging the code.
