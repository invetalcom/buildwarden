...
import { getDefaultDatabasePath, EasycodeDatabase } from "@easycode/db";
import {
  APP_SETTING_KEYS,
  IPC_CHANNELS,
...
const PROD_DB_FILE_NAME = "easycode.sqlite";
const DEV_DB_FILE_NAME = "easycode_dev.sqlite";
...
try {
  ...
  console.error("[easycode:error] Failed to set Electron logs path", error);
}
  ...
  return [
    ...
    },
    {
      ...
      submenu: [
        ...
        { type: "separator" },
        {
          label: "About Easycode",
          click: async () => {
            await dialog.showMessageBox({
              type: "info",
              title: "About Easycode",
              message: "Easycode",
...
const bootstrap = async (): Promise<void> => {
  ...
  logInfo("Bootstrapping Easycode main process.", {
    userDataPath,
    logDirPath,
  ...
  const db = new EasycodeDatabase(getDefaultDatabasePath(dataDirPath, databaseFileName));
  await db.init();
  ...
  ipcMain.handle(IPC_CHANNELS.startProjectLabImplementation, (_, threadId: string) => controller.startProjectLabImplementation(threadId));
  ...
  ipcMain.handle(IPC_CHANNELS.deleteProject, (_, projectId: string) => controller.deleteProject(projectId));
...
app.whenReady().then(async () => {
  try {
    ...
    logError("Failed to start Easycode.", { error: err });
    await dialog.showMessageBox({
      type: "error",
      title: "Startup Error",
      message: "Failed to start Easycode",
