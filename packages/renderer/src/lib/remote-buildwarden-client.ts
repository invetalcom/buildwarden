...
const REMOTE_MUTATION_SCOPES = new Map<RemoteApiMethod, readonly RemoteAccessScope[]>([
  ...[
    ...
    "removeBookmarkById", "addRunNote", "updateRunNote", "deleteRunNote",
    "pauseOrchestration", "resumeOrchestration", "cancelOrchestration", "finishOrchestration",
    "sendOrchestrationTaskMessage", "retryOrchestrationTask",
  ...
  ...[
    ...
    "resolveProjectPrMrReviewThread", "fetchProjectPrMrDiff", "updateProjectForgeRequest", "mergeProjectForgeRequest",
    "updateRunForgeRequest", "mergeRunForgeRequest",
  ...
  ...[
    ...
    "generateProjectTaskRunPrompt", "createProjectAutomation", "updateProjectAutomation", "deleteProjectAutomation",
    "runProjectAutomationNow", "generateProjectInsight", "runProjectLab", "deleteProjectLabThread", "createProjectLoop",
      ...
      if (REMOTE_MUTATION_METHODS.has(property as RemoteApiMethod)) {
        ...
        if (missingScopes.length > 0) {
          return async () => {
            ...
            throw new Error(`"${property}" is not available for ${reason}; missing ${missingScopes.join(", ")}.`);
          };
        }
        return (...args: unknown[]) => invoke(property as RemoteApiMethod, args as never);
      }
